/** Response shapes from the WHOOP API v2 (https://api.prod.whoop.com/developer). */

/** WHOOP scores a record asynchronously; only SCORED records carry a `score`. */
export type ScoreState = "SCORED" | "PENDING_SCORE" | "UNSCORABLE";

export interface Paged<T> {
  records: T[];
  /** Cursor for the next page. Sent back as the `nextToken` query param. */
...
export interface Cycle {
  ...
  updated_at: string;
  start: string;
  /** Absent while the cycle is still in progress (i.e. today). */
  end?: string | null;
